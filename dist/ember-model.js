  ...
  _reloadHasManys: function(reverting) {
    ...
    for (i = 0; i < this._hasManyArrays.length; i++) {
      var array = this._hasManyArrays[i],
          embedded = get(array, 'embedded'),
          hasManyContent = this._getHasManyContent(get(array, 'key'), get(array, 'modelClass'), embedded);
      if (!reverting && !embedded) {
...
Ember.Model.Store = Ember.Object.extend({
  ...
  modelFor: function(type) {
    return Ember.get(Ember.lookup, type) || this.container.lookupFactory('model:'+type);
  },
